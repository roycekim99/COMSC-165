...
void buySeat(vector<string> &inputVector, int maxSeats, 
    ...
    int rowNum;
    string seatChars = "";
    string seatString = userInput.substr(sPos + 1, 
                                         userInput.size() - sPos);
    ...
    if (userInput.at(0) == 'R' && sPos &&
        ...
        if (seatIsThere(inputVector, maxSeats, rowNum, seatNum)) {
            inputVector.at(rowNum - 1).replace(
                seatNum - 1, seatChars.size(), seatChars
            );
        ...
        case 2: //find a seat
            ...
            if (!foundSeats.compare("none")) {  //if no seats were found
                ...
                if (toupper(userInput.at(0)) == 'Y') {
                    ...
                    buySeat(theater, maxSeats, 
                            foundSeats, requestedSeats
                    );
...
vector<string> scalePrices(const vector<string> &originalPrices, 
    ...
    size = (originalPrices.size() > newRowMax) ?
...
void setupWizard(vector<string> &theater, 
    ...
    //initialize prices
    initializeSeatPrices(tempPriceList, requested, newRowMax, false); 
    ...
    do {
        ...
        cout << "Enter a list of special rows and prices " <<
            ...
            endl << "ex: (R10$20 R2$21 R30$2)" << endl <<
...
void setupMenu(vector<string> &theater, int &currentMaxRows, 
    ...
    while (userCommand != 0) {
        ...
        case 3:
            ...
            (userCommand == 2) ? 
                newRowMax = requested : newSeatmax = requested;
